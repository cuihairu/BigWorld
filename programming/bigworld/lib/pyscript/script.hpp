...
/**
 *	Borrowed-reference replacement for bwPyWeakrefGetObject, which is
...
#define INT_ACCESSOR( INPUT_TYPE, COMMON_TYPE )					\
	...
	// BIGWORLD_BEGIN(3.13 migration)
	// IsValidRetData<>::getData resolves its qualified Script::getData() call
	// in this header's context, where only the built-in overloads (bool, int,
	// Vector3, ...) are visible. The class-specific overloads generated by
	// PY_SCRIPT_CONVERTERS() are declared later and never join that overload
	// set, so every RETDATA function returning a SmartPointer<CLASS> used to
	// fall back to getData( const bool ) - via the safe-bool operator - and
	// handed Python True/False instead of the wrapped object (e.g.
	// Math.getRegister() returned a bool, which then failed every
	// Vector4Provider argument conversion). Route smart pointers straight to
	// the built-in PyObject overload instead, which increfs the same object
	// and maps NULL to None.
	template <class T>
	PyObject * getRetData( const SmartPointer<T> & data )
	{
		return getData( static_cast< const PyObject * >( data.get() ) );
	}
	// BIGWORLD_END
...
#define PY_ENUM_CONVERTER_SET( ENUMTYPE, CONTAINER_NAME )					\
	...
	{																		\
		OrderedStringMap<ENUMTYPE>::iterator iter = CONTAINER_NAME##_smap.end();	\
																			\
		/* BIGWORLD_BEGIN(3.13 migration): was PyUnicode_Check/			\
		 * PyUnicode_AsUTF8. */											\
...
public:
	...
	const PyObject * getPyObj() const
	{
		if (uintptr(weakref_) > 1)
		{
			...
			// BIGWORLD_BEGIN(3.13 migration)
			// Was bwPyWeakrefGetObject (deprecated).
...
{
	// BIGWORLD_BEGIN(3.13 migration)
	// Was PyLong_Check on _WIN64 and PyLong_Check elsewhere; PyInt has
...
{
	// BIGWORLD_BEGIN(3.13 migration)
	// Was PyLong_AsLong on _WIN64 and PyLong_AsLong elsewhere.
